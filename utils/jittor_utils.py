...
def find_modules(model, mclass=nn.Conv):
...
def fuse_conv_and_bn(conv, bn):
    ...
    with jt.no_grad():
        # init
        fusedconv = nn.Conv(conv.in_channels,
                                    conv.out_channels,
        ...
        fusedconv.weight = nn.matmul(w_bn, w_conv).view(fusedconv.weight.size())

        # prepare spatial bias
        if conv.bias is not None:
            b_conv = conv.bias
        else:
            b_conv = jt.zeros((conv.weight.size(0),))
        b_bn = bn.bias - ((bn.weight * bn.running_mean) / jt.sqrt(bn.running_var + bn.eps))
        fusedconv.bias = nn.matmul(w_bn, b_conv.reshape(-1, 1)).reshape(-1) + b_bn
...
def model_info(model, verbose=False):
    ...
    n_g = sum(x.numel() for x in model.parameters() if not x.is_stop_grad())  # number gradients
    if verbose:
    ...
    try:  # FLOPS
        ...
        macs, _ = profile(model, inputs=(jt.zeros((1, 3, 480, 640)),), verbose=False)
...
def load_classifier(name='resnet101', n=2):
    ...
    model.last_linear.bias = jt.zeros((n,))
    model.last_linear.weight = jt.zeros((n, filters))
