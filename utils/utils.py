...
def init_seeds(seed=0):
    random.seed(seed)
    np.random.seed(seed)
    jittor_utils.init_seeds(seed=seed)
...
def clip_coords(boxes, img_shape):
    ...
    boxes[:, 0] = jt.clamp(boxes[:, 0], 0, img_shape[1])  # x1
    boxes[:, 1] = jt.clamp(boxes[:, 1], 0, img_shape[0])  # y1
    boxes[:, 2] = jt.clamp(boxes[:, 2], 0, img_shape[1])  # x2
    boxes[:, 3] = jt.clamp(boxes[:, 3], 0, img_shape[0])  # y2
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    # Intersection area
    inter = (jt.minimum(b1_x2, b2_x2) - jt.maximum(b1_x1, b2_x1)).clamp(min_v=0) * \
            (jt.minimum(b1_y2, b2_y2) - jt.maximum(b1_y1, b2_y1)).clamp(min_v=0)
...
def box_iou(box1, box2):
    ...
    inter = (jt.minimum(box1[:, 2:].unsqueeze(1), box2[:, 2:]) - jt.maximum(box1[:, :2].unsqueeze(1), box2[:, :2])).clamp(min_v=0).prod(2)
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    ft = jt.array
    lcls, lbox, lobj = ft([0],dtype='float32'), ft([0],dtype='float32'), ft([0],dtype='float32')
    ...
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=ft([h['cls_pw']]), size_average=True if red == 'mean' else False)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=ft([h['obj_pw']]), size_average=True if red == 'mean' else False)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            # GIoU
            pxy = ps[:, :2].sigmoid()
            pwh = ps[:, 2:4].exp().clamp(max_v=1E3) * anchors[i]
            pbox = jt.contrib.concat((pxy, pwh), 1)  # predicted box
            giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=True)  # giou(prediction, target)
            lbox += (1.0 - giou).sum() if red == 'sum' else (1.0 - giou).mean()  # giou loss

            # Obj
            tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clamp(min_v=0).cast(tobj.dtype)  # giou ratio
    ...
    loss = lbox + lobj + lcls
    return loss, jt.contrib.concat((lbox, lobj, lcls, loss),dim=0).detach()
...
def build_targets(p, targets, model):
    ...
    gain = jt.ones((6,))  # normalized to gridspace gain
    off = jt.array([[1, 0], [0, 1], [-1, 0], [0, -1]]).float()  # overlap offsets

    style = None
    multi_gpu = False # type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)
    for i, j in enumerate(model.yolo_layers):
        anchors = model.module.module_list[j].anchor_vec if multi_gpu else model.module_list[j].anchor_vec
        gain[2:] = jt.array(list(p[i].shape))[jt.array([3, 2, 3, 2])]  # xyxy gain
        na = anchors.shape[0]  # number of anchors
        at = jt.arange(na).view(na, 1).repeat(1, nt)  # anchor tensor, same as .repeat_interleave(nt)
        ...
        if nt:
            ...
            if style == 'rect2':
                g = 0.2  # offset
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T
                a, t = jt.contrib.concat((a, a[j], a[k]), 0), jt.contrib.concat((t, t[j], t[k]), 0)
                offsets = jt.contrib.concat((z, z[j] + off[0], z[k] + off[1]), 0) * g

            elif style == 'rect4':
                g = 0.5  # offset
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T
                l, m = ((gxy % 1. > (1 - g)) & (gxy < (gain[[2, 3]] - 1.))).T
                a, t = jt.contrib.concat((a, a[j], a[k], a[l], a[m]), 0), jt.contrib.concat((t, t[j], t[k], t[l], t[m]), 0)
                offsets = jt.contrib.concat((z, z[j] + off[0], z[k] + off[1], z[l] + off[2], z[m] + off[3]), 0) * g

        # Define
        b, c = t[:, :2].int32().t()[0], t[:, :2].int32().t()[1]  # image, class
        gxy = t[:, 2:4]  # grid xy
        gwh = t[:, 4:6]  # grid wh
        gij = (gxy - offsets).int32()
        gi, gj = gij.t()[0], gij.t()[1]  # grid xy indices
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi in range(prediction.shape[0]):  # image index, image inference
        ...
        if multi_label:
            ...
            i, j = tmp[0],tmp[1]
            x = jt.contrib.concat((box[i], x[i, j + 5].unsqueeze(1), j.float().unsqueeze(1)), 1)
        else:  # best class only
            j,conf = x[:, 5:].argmax(1)
            x = jt.contrib.concat((box, conf.unsqueeze(1), j.float().unsqueeze(1)), 1)[conf > conf_thres]

        # Filter by class
        if classes:
            x = x[(j.view(-1, 1) == jt.array(classes)).any(1)]
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            try:  # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                ...
                weights = iou * scores.unsqueeze(0)  # box weights
                x[i, :4] = nn.matmul(weights, x[:, :4]).float() / weights.sum(1, keepdims=True)  # merged boxes
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2,_ = model(jt.array(ims)).argmax(1)  # classifier prediction
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if isinstance(x,jt.Var):
        x = x.numpy()
    c1, c2 = (int(x[0].item()), int(x[1].item())), (int(x[2].item()), int(x[3].item()))
