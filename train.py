...
def train(hyp):
    ...
    if opt.adam:
        ...
        optimizer = optim.SGD([{'params':pg0},{'params': pg1, 'weight_decay': hyp['weight_decay']},{'params': pg2}], lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        # load model
        try:
            ckpt['model'] = {k: v for k, v in ckpt['model'].items() if k in model.state_dict() and model.state_dict()[k].numel() == v.numel()}
    ...
    dataloader = LoadImagesAndLabels(train_path, img_size, batch_size,
                                  augment=True,
    ...
    # Testloader
    testloader = LoadImagesAndLabels(test_path, imgsz_test, batch_size,
                                                                 hyp=hyp,
                                                                 rect=True,
                                                                 cache_images=opt.cache_images,
                                                                 single_cls=opt.single_cls,
    ...
    model.class_weights = labels_to_class_weights(dataloader.labels, nc)  # attach class weights

    # Model EMA
    ema = jittor_utils.ModelEMA(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if dataloader.image_weights:
            w = model.class_weights.numpy() * (1 - maps) ** 2  # class weights
            image_weights = labels_to_image_weights(dataloader.labels, nc=nc, class_weights=w)
            dataloader.indices = random.choices(range(dataloader.n), weights=image_weights, k=dataset.n)  # rand weighted idx
